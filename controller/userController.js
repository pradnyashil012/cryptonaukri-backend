...
exports.sendOTP = async (req, res) => {
  const userPresenceCheck = await userDatabase.findOne({
      ...
      if (Number(data) === req.body.otp) {
        if (req.query.coupon) {
          ...
          if (couponData.referredUserEmail.length < 3) {
            const userAssociatedWithCoupon = await userDatabase.findOne({
              email: couponData.userAssociated,
            });
            userAssociatedWithCoupon.accountDisableDate = new Date(
              userAssociatedWithCoupon.accountDisableDate.setDate(
                userAssociatedWithCoupon.accountDisableDate.getDate() + 14
              )
            );
            await userDatabase.findByIdAndUpdate(
              userAssociatedWithCoupon._id,
        ...
        if (req.query.bonusCoupon) {
          const customCouponData = await customCouponDatabase.findOne({
            finalCoupon: req.query.bonusCoupon,
          });
          if (customCouponData) {
            if (
              customCouponData.usersAssociated.length <
              customCouponData.numberOfUsers
            ) {
              await customCouponDatabase.findByIdAndUpdate(
                customCouponData._id,
                {
                  $push: {
              ...
              userDataToBeSaved.accountDisableDate =
                Date.now() +
                customCouponData.numberOfDays * 24 * 60 * 60 * 1000;
            }
          }
        }
        /*
  ...
  try {
    ...
    if (user) {
      if (user.isDisabled) {
        ...
        const userResume = await userResumeDatabase.findOne({
  ...
  if (req.user.ROLE === "USER") {
    try {
      const {
        ...
        accountDisableDate,
        _id,
      } = await userDatabase.findById(req.user._id);
      const appliedAtJobs = await userAnswersDatabase.find({
      ...
      const appliedAtInternships = await userAnswersInternshipDatabase.find({
        userAssociated: req.user._id,
        isDisabled: false,
      });
...
exports.userResumeUpdate = async (req, res) => {
  try {
    const updatedResume = await userResumeDatabase.findOneAndUpdate(
      { userAssociated: req.user._id },
      req.body,
      { new: true }
    );
...
exports.userProfileUpdate = async (req, res) => {
  try {
    const updatedUserProfile = await userDatabase.findByIdAndUpdate(
      req.user._id,
      req.body,
      { new: true }
    );
