...
const oAuth2Client = new google.auth.OAuth2(
  ...
  `https://www.cryptonaukri.com/auth/google`
);
...
exports.githubUserInfo = async (req, res) => {
  try {
    ...
    const accessTokenResponse = await axios.post(
      `https://github.com/login/oauth/access_token?client_id=${process.env.GITHUB_CLIENT_ID}&client_secret=${process.env.GITHUB_CLIENT_SECRET}&redirect_url=${process.env.GITHUB_REDIRECT}&code=${code}`,
      {
        headers: {
    ...
    await signUpOrSignInUser(
      userDataEmail.data[0].email,
      userData.data.name,
      req,
      res
    );
...
exports.linkedinOAuthCall = async (req, res) => {
  try {
    const data = await axios.get(
      `https://www.linkedin.com/oauth/v2/authorization?response_type=code&client_id=${process.env.LINKEDIN_CLIENT_ID}&redirect_uri=${process.env.LINKEDIN_REDIRECT}&scope=r_liteprofile%20r_emailaddress&state`
    );
    return res.status(200).json({
      message: "OAuth Redirect URL",
      code: "OAUTH_SUCCESS",
...
exports.linkedinUserInfo = async (req, res) => {
  try {
    ...
    // console.log(code);
    const accessTokenResponse = await axios.post(
      `https://www.linkedin.com/oauth/v2/accessToken?client_id=${process.env.LINKEDIN_CLIENT_ID}&client_secret=${process.env.LINKEDIN_CLIENT_SECRET}&redirect_uri=${process.env.LINKEDIN_REDIRECT}&code=${code}&grant_type=authorization_code`,
      {
        headers: {
